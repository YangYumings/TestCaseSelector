...
class CIPointWiseEnv(gym.Env, ABC):
    def __init__(self, config, cycle_test_cases):
        self.conf = config
        self.reward_range = (-1, 1)  # 奖励范围
        self.cycle_logs = cycle_test_cases
        self.test_cases_vector_prob = []  # 测试用例概率
        self.current_index = 0
        self.optimal_order = cycle_test_cases.get_optimal_order()  # 获取最优测试用例排序顺序
        self.testcase_vector_size = self.cycle_logs.get_test_case_vector_length(cycle_test_cases.test_cases[0],
                                                                                self.conf.win_size)
