...
class Agent:
    def __init__(self):
        self.supported_algo = ['DQN', 'PPO2', "A2C", "ACKTR", "DDPG", "ACER", "GAIL", "HER", "PPO1", "SAC", "TD3",
                               "TRPO"]

    def create_model(self, algo, env):
        # count 是统计其在列表中出现的次数
        global model
        assert self.supported_algo.count(algo.upper()) == 1, "The algorithms is not supported for" \
    ...
    def load_model(self, algo, env, path):
        if algo.upper() == "DQN":
    ...
    def test_agent(self, env, model_path: str, algo, mode):
        agent_actions = []
        print("Evaluation of an agent from " + model_path)
        agent_instance = Agent()
        model = agent_instance.load_model(path=model_path, algo=algo, env=env)
        if model:
            if mode.upper() == "PAIRWISE" and algo.upper() != "DQN":
                ...
                if model:
                    ...
                    for index in range(0, len(test_cases)):
                        ...
                        # print(action)
                        obs, rewards, done, info = env.step(action)
                        verdict = test_cases[index]['verdict']
                        test_cases_vector_prob.append({'index': index, 'prob': action, 'verdict': verdict})
                        if done:
                    ...
                    for i, test_case_info in enumerate(test_cases_vector_prob):
                        original_test_case = test_cases[test_case_info['index']]
                        # 向原始测试用例对象添加概率值
                        original_test_case['prob'] = test_case_info['prob']

                    # for test_case in test_cases_vector_prob:
                    #     sorted_test_cases.append(test_cases[test_case['index']])
                    for test_case in test_cases_vector_prob:
                        test_case_with_prob = test_cases[test_case['index']].copy()  # 创建副本避免修改原数据
                        test_case_with_prob['prob'] = test_case['prob']
                        sorted_test_cases.append(test_case_with_prob)
